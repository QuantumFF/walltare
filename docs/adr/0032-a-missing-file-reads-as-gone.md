...
> Review strip share, so both show the same two lines. It sits in the picture's
> box after the two images rather than in a grid cell, and still covers the
> failed request with no z-index while the row stays where it was. The reset per
